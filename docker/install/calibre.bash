...
# Compose-Datei
compose_file="$config_dir/calibre.yml"

# Service-Datei
service_file="/etc/systemd/system/calibre.service"
...
run_command "mkdir -p $container_dir/calibre/config" "$sudo_available"
...
# Überprüfen, ob Calibre bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^calibre$"; then
    echo "Calibre ist bereits installiert."
else
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        port1=$(find_next_port 8080)
        port2=$(find_next_port 8181)
        port3=$(find_next_port 8081)
...
services:
  calibre:
    ...
    environment:
      ...
      - CLI_ARGS= #optional
    volumes:
      - $container_dir/calibre/config:/config
    ports:
      - "$port1:8080"
      - "$port2:8181"
      - "$port3:8081"
