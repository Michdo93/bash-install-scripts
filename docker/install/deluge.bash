...
# Compose-Datei
compose_file="$config_dir/deluge.yml"

# Service-Datei
service_file="/etc/systemd/system/deluge.service"
...
run_command "mkdir -p $container_dir/deluge/config:/config" "$sudo_available"
run_command "mkdir -p $container_dir/deluge/downloads:/downloads" "$sudo_available"
...
# Überprüfen, ob deluge bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^deluge$"; then
    echo "deluge ist bereits installiert."
else
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        port1=$(find_next_port 8112)
        port2=$(find_next_port 6881)
        port3=$(find_next_port 58846)
...
services:
  deluge:
    ...
    volumes:
      ...
      - $container_dir/deluge/downloads:/downloads
    ports:
      - "$port1:8112"
      - "$port2:6881"
      - "$port2:6881/udp"
      - "$port3:58846" #optional
...
sudo systemctl enable deluge.service
sudo systemctl start deluge.service
