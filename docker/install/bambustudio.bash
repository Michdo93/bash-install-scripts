...
# Compose-Datei
compose_file="$config_dir/bambustudio.yml"

# Service-Datei
service_file="/etc/systemd/system/bambustudio.service"
...
run_command "mkdir -p $container_dir/bambustudio/config" "$sudo_available"
...
# Überprüfen, ob BambooStudio bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^bambustudio$"; then
    echo "BambooStudio ist bereits installiert."
else
...
services:
  bambustudio:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/bambustudio/config:/config
    ports:
      - "$port1:3000"
      - "$port2:3001"
...
sudo systemctl enable bambustudio.service
sudo systemctl start bambustudio.service
