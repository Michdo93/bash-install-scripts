...
run_command "mkdir -p $container_dir/duplicati/config" "$sudo_available"
run_command "mkdir -p $container_dir/duplicati/backups" "$sudo_available"
run_command "mkdir -p $container_dir/duplicati/source" "$sudo_available"
...
if docker ps -a --format '{{.Names}}' | grep -q "^duplicati$"; then
    ...
    if [ ! -f "$compose_file" ]; then
        ...
        available_port=$(find_next_port 8200)
...
services:
  duplicati:
    ...
    environment:
      ...
      - CLI_ARGS= #optional
    volumes:
      - $container_dir/duplicati/config:/config
      - $container_dir/duplicati/backups:/backups
      - $container_dir/duplicati/source:/source
    ports:
      - "$available_port:8200"
