...
# Überprüfen, ob Wireshark bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^wireshark$"; then
    echo "Wireshark ist bereits installiert."
else
    # Installieren und Starten von Wireshark
    sudo_available=$(check_sudo)
    run_command "docker run -d --name=wireshark --cap-add=NET_ADMIN --security-opt seccomp:unconfined --network host --restart=unless-stopped -e PUID=1000 -e PGID=1000 -e TZ=Etc/UTC -v $config_dir/wireshark:/config -p 3000:3000 -p 3001:3001 lscr.io/linuxserver/wireshark:latest" "$sudo_available"
fi
...
compose_file="$config_dir/wireshark.yml"
...
services:
  wireshark:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/wireshark/config:/config
    ports:
      - "$available_port:3000" #optional
      - 3001:3001 #optional
...
service_file="/etc/systemd/system/wireshark-setup.service"
cat > "$service_file" <<EOL
[Unit]
Description=Wireshark Setup
...
sudo systemctl enable wireshark-setup.service
sudo systemctl start wireshark-setup.service
