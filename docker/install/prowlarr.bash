...
# Compose-Datei
compose_file="$config_dir/prowlarr.yml"

# Service-Datei
service_file="/etc/systemd/system/prowlarr.service"
...
# Überprüfen, ob Prowlarr bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^prowlarr$"; then
    echo "Prowlarr ist bereits installiert."
else
    # Prowlarr Docker Compose-Datei erstellen
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 9696)
...
EOL
        ...
        echo "Prowlarr Docker Compose-Datei erstellt: $compose_file"

        # Prowlarr-Dienst starten
        run_command "docker-compose -f $compose_file up -d" "$sudo_available"
    else
        echo "Die Prowlarr Docker Compose-Datei existiert bereits: $compose_file"
    fi
fi
