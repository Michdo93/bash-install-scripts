...
# Compose-Datei
compose_file="$config_dir/xbackbone.yml"

# Service-Datei
service_file="/etc/systemd/system/xbackbone-setup.service"
...
# Überprüfen, ob xBackbone bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^xbackbone$"; then
    echo "xBackbone ist bereits installiert."
else
    # Installieren und Starten von xBackbone
    sudo_available=$(check_sudo)
    run_command "docker run -d --name=xbackbone --restart=unless-stopped -e PUID=1000 -e PGID=1000 -e TZ=Etc/UTC -v $config_dir/xbackbone:/config -p 80:80 -p 443:443 lscr.io/linuxserver/xbackbone:latest" "$sudo_available"
fi
...
services:
  xbackbone:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/xbackbone/config:/config
    ports:
      - "$available_port:80"
      - 443:443
...
sudo systemctl enable xbackbone-setup.service
sudo systemctl start xbackbone-setup.service
