...
# Compose-Datei
compose_file="$config_dir/duckdns.yml"
...
run_command "mkdir -p $container_dir/duckdns/config" "$sudo_available"
...
# Überprüfen, ob DuckDNS bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^duckdns$"; then
    echo "DuckDNS ist bereits installiert."
else
...
services:
  duckdns:
    ...
    volumes:
      - $container_dir/duckdns/config:/config #optional
