...
# Compose-Datei
compose_file="$config_dir/davos.yml"

# Service-Datei
service_file="/etc/systemd/system/davos.service"
...
run_command "mkdir -p $container_dir/davos/config" "$sudo_available"
run_command "mkdir -p $container_dir/davos/downloads" "$sudo_available"
...
# Überprüfen, ob davos bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^davos$"; then
    echo "davos ist bereits installiert."
else
...
services:
  davos:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/davos/config:/config
      - $container_dir/davos/downloads:/download
    ports:
      - "$available_port:8080"
...
sudo systemctl enable davos.service
sudo systemctl start davos.service
