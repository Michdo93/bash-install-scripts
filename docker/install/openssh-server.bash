...
# Compose-Datei
compose_file="$config_dir/openssh-server.yml"

# Service-Datei
service_file="/etc/systemd/system/openssh-server-setup.service"
...
# Überprüfen, ob openssh-server bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^openssh-server$"; then
    echo "openssh-server ist bereits installiert."
else
    # openssh-server Docker Compose-Datei erstellen
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 2222)
...
services:
  openssh-server:
    ...
    environment:
      ...
      - LOG_STDOUT=
    volumes:
      - $container_dir/openssh-server/config:/config
    ports:
      - "$available_port:2222"
    restart: unless-stopped
EOL

        echo "openssh-server Docker Compose-Datei erstellt."
    else
        echo "Die openssh-server Docker Compose-Datei existiert bereits."
    fi
...
[Unit]
Description=OpenSSH Server Setup
...
EOL
    ...
    sudo systemctl enable openssh-server-setup.service
    sudo systemctl start openssh-server-setup.service
fi
