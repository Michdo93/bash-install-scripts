...
# Compose-Datei
compose_file="$config_dir/dokuwiki.yml"

# Service-Datei
service_file="/etc/systemd/system/dokuwiki.service"
...
run_command "mkdir -p $container_dir/dokuwiki" "$sudo_available"
...
# Überprüfen, ob dokuwiki bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^dokuwiki$"; then
    echo "DokuWiki ist bereits installiert."
else
...
sudo systemctl enable dokuwiki.service
sudo systemctl start dokuwiki.service
