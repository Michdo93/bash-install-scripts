...
# Compose-Datei
compose_file="$config_dir/syslog-ng.yml"

# Service-Datei
service_file="/etc/systemd/system/syslog-ng.service"
...
# Überprüfen, ob syslog-ng bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^syslog-ng$"; then
    echo "syslog-ng ist bereits installiert."
else
    # syslog-ng Docker Compose-Datei erstellen
...
EOL
        ...
        echo "Die syslog-ng Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei nach Bedarf an."
    else
        echo "Die Docker Compose-Datei für syslog-ng existiert bereits: $compose_file"
    fi
fi
