...
# Überprüfen, ob WebTop bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^webtop$"; then
    echo "WebTop ist bereits installiert."
else
    # Installieren und Starten von WebTop
    sudo_available=$(check_sudo)
    run_command "docker run -d --name=webtop --restart=always -v $config_dir/webtop:/config -v /var/run/docker.sock:/var/run/docker.sock lscr.io/linuxserver/webtop:latest" "$sudo_available"
fi
...
compose_file="$config_dir/webtop-setup.yml"
...
services:
  webtop:
    ...
    volumes:
      - $container_dir/webtop/config:/config
...
service_file="/etc/systemd/system/webtop-setup.service"
cat > "$service_file" <<EOL
[Unit]
Description=WebTop Setup
...
sudo systemctl enable webtop-setup.service
sudo systemctl start webtop-setup.service
