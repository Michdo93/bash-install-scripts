...
# Compose-Datei
compose_file="$config_dir/openvscode-server.yml"

# Service-Datei
service_file="/etc/systemd/system/openvscode-server-setup.service"
...
# Überprüfen, ob openvscode-server bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^openvscode-server$"; then
    echo "openvscode-server ist bereits installiert."
else
    # openvscode-server Docker Compose-Datei erstellen
...
services:
  openvscode-server:
    ...
    environment:
      ...
      - SUDO_PASSWORD_HASH=
    volumes:
      - $container_dir/openvscode-server/config:/config
    ports:
      - "$available_port:3000"
    restart: unless-stopped
EOL

        echo "openvscode-server Docker Compose-Datei erstellt."
    else
        echo "Die openvscode-server Docker Compose-Datei existiert bereits."
    fi
...
[Unit]
Description=OpenVSCode Server Setup
...
EOL
    ...
    sudo systemctl enable openvscode-server-setup.service
    sudo systemctl start openvscode-server-setup.service
fi
