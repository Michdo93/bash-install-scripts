...
# Compose-Datei
compose_file="$config_dir/rsnapshot.yml"

# Service-Datei
service_file="/etc/systemd/system/rsnapshot.service"
...
# Überprüfen, ob rsnapshot bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^rsnapshot$"; then
    echo "rsnapshot ist bereits installiert."
else
    # rsnapshot Docker Compose-Datei erstellen
...
services:
  rsnapshot:
    ...
    volumes:
      ...
      - /path/to/snapshots:/.snapshots
      - /path/to/data:/data
    restart: unless-stopped
EOL

        echo "Die rsnapshot Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei nach Bedarf an."
    else
        echo "Die Docker Compose-Datei für rsnapshot existiert bereits: $compose_file"
    fi
...
sudo systemctl enable rsnapshot.service
sudo systemctl start rsnapshot.service
