...
# Compose-Datei
compose_file="$config_dir/dillinger.yml"

# Service-Datei
service_file="/etc/systemd/system/dillinger.service"
...
run_command "mkdir -p $container_dir/dillinger/configs" "$sudo_available"
...
# Überprüfen, ob dillinger bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^dillinger$"; then
    echo "dillinger ist bereits installiert."
else
...
services:
  dillinger:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/dillinger/config:/config
    ports:
      - "$available_port:8080"
...
sudo systemctl enable dillinger.service
sudo systemctl start dillinger.service
