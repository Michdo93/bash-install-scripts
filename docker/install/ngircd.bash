...
# Compose-Datei
compose_file="$config_dir/ngircd.yml"

# Service-Datei
service_file="/etc/systemd/system/ngircd-setup.service"
...
# Überprüfen, ob ngIRCd bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^ngircd$"; then
    echo "ngIRCd ist bereits installiert."
else
    # ngIRCd Docker Compose-Datei erstellen
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 6667)
...
services:
  ngircd:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $config_dir/ngircd/config:/config
    ports:
      - "$available_port:6667"
    restart: unless-stopped
EOL

        echo "Docker Compose-Datei für ngIRCd erstellt."
    else
        echo "Die Docker Compose-Datei für ngIRCd existiert bereits."
    fi
...
EOL
    ...
    sudo systemctl enable ngircd-setup.service
    sudo systemctl start ngircd-setup.service
fi
