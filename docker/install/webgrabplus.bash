...
# Überprüfen, ob WebGrab+Plus bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^webgrabplus$"; then
    echo "WebGrab+Plus ist bereits installiert."
else
    # Installieren und Starten von WebGrab+Plus
    sudo_available=$(check_sudo)
    run_command "docker run -d --name=webgrabplus --restart=always -v $config_dir/webgrabplus/config:/config -v $config_dir/webgrabplus/data:/data lscr.io/linuxserver/webgrabplus:latest" "$sudo_available"
fi
...
compose_file="$config_dir/webgrabplus-setup.yml"
...
services:
  webgrabplus:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $config_dir/webgrabplus/config:/config
      - $config_dir/webgrabplus/data:/data
...
service_file="/etc/systemd/system/webgrabplus-setup.service"
cat > "$service_file" <<EOL
[Unit]
Description=WebGrab+Plus Setup
...
sudo systemctl enable webgrabplus-setup.service
sudo systemctl start webgrabplus-setup.service
