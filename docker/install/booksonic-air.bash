...
# Compose-Datei
compose_file="$config_dir/booksonic-air.yml"

# Service-Datei
service_file="/etc/systemd/system/booksonic-air.service"
...
run_command "mkdir -p $container_dir/booksonic-air/config" "$sudo_available"
run_command "mkdir -p $container_dir/booksonic-air/audiobooks" "$sudo_available"
run_command "mkdir -p $container_dir/booksonic-air/podcasts" "$sudo_available"
run_command "mkdir -p $container_dir/booksonic-air/othermedia" "$sudo_available"
...
# Überprüfen, ob Booksonic Air bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^booksonic-air$"; then
    echo "Booksonic Air ist bereits installiert."
else
...
services:
  booksonic-air:
    ...
    volumes:
      - $container_dir/booksonic-air/config:/config
      - $container_dir/booksonic-air/audiobooks:/audiobooks
      - $container_dir/booksonic-air/podcasts:/podcasts
      - $container_dir/booksonic-air/othermedia:/othermedia
    ports:
      - "$available_port:4040"
...
sudo systemctl enable booksonic-air.service
sudo systemctl start booksonic-air.service
