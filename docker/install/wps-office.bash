...
# Überprüfen, ob WPS Office bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^wps-office$"; then
    echo "WPS Office ist bereits installiert."
else
    # Installieren und Starten von WPS Office
    sudo_available=$(check_sudo)
    run_command "docker run -d --name=wps-office --security-opt seccomp:unconfined --restart=unless-stopped -e PUID=1000 -e PGID=1000 -e TZ=Etc/UTC -v $config_dir/wps-office:/config -p 3000:3000 -p 3001:3001 --shm-size=1gb lscr.io/linuxserver/wps-office:latest" "$sudo_available"
fi
...
compose_file="$config_dir/wps-office.yml"
...
services:
  wps-office:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/wps-office/config:/config
    ports:
      - "$available_port:3000"
      - 3001:3001
    shm_size: "1gb"
...
service_file="/etc/systemd/system/wps-office-setup.service"
cat > "$service_file" <<EOL
[Unit]
Description=WPS Office Setup
...
sudo systemctl enable wps-office-setup.service
sudo systemctl start wps-office-setup.service
