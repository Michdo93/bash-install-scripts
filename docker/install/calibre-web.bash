...
# Compose-Datei
compose_file="$config_dir/calibre-web.yml"
...
run_command "mkdir -p $container_dir/calibre-web/config" "$sudo_available"
run_command "mkdir -p $container_dir/calibre-web/books" "$sudo_available"
...
# Überprüfen, ob Calibre-Web bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^calibre-web$"; then
    echo "Calibre-Web ist bereits installiert."
else
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 8083)
...
services:
  calibre-web:
    ...
    volumes:
      - $container_dir/calibre-web/config:/config
      - $container_dir/calibre-web/library:/books
    ports:
      - "$available_port:8083"
