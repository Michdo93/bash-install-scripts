...
# Compose-Datei
compose_file="$config_dir/code-server.yml"
...
run_command "mkdir -p $container_dir/code-server/config" "$sudo_available"
...
# Überprüfen, ob code-server bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^code-server$"; then
    ...
    if [ ! -f "$compose_file" ]; then
        ...
        available_port=$(find_next_port 8443)
...
services:
  code-server:
    ...
    volumes:
      - $container_dir/code-server/config:/config
    ports:
      - "$available_port:8443"
