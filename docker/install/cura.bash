...
# Compose-Datei
compose_file="$config_dir/cura.yml"

# Service-Datei
service_file="/etc/systemd/system/cura.service"
...
run_command "mkdir -p $container_dir/cura/config" "$sudo_available"
...
# Überprüfen, ob cura bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^cura$"; then
    echo "cura ist bereits installiert."
else
...
services:
  cura:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/cura/config:/config
    ports:
      - "$port1:3000"
      - "$port2:3001"
...
sudo systemctl enable cura.service
sudo systemctl start cura.service
