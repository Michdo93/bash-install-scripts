...
# Compose-Datei
compose_file="$config_dir/snapdrop.yml"

# Service-Datei
service_file="/etc/systemd/system/snapdrop.service"
...
# Überprüfen, ob Snapdrop bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^snapdrop$"; then
    echo "Snapdrop ist bereits installiert."
else
    # Snapdrop Docker Compose-Datei erstellen
...
EOL
        ...
        echo "Die Snapdrop Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei nach Bedarf an."
    else
        echo "Die Docker Compose-Datei für Snapdrop existiert bereits: $compose_file"
    fi
fi
...
sudo systemctl enable snapdrop.service
sudo systemctl start snapdrop.service
