...
# Compose-Datei
compose_file="$config_dir/daapd.yml"

# Service-Datei
service_file="/etc/systemd/system/daapd.service"
...
run_command "mkdir -p $container_dir/daapd/config" "$sudo_available"
run_command "mkdir -p $container_dir/daapd/music" "$sudo_available"
...
# Überprüfen, ob daapd bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^daapd$"; then
    echo "daapd ist bereits installiert."
else
...
services:
  daapd:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/daapd/config:/config
      - $container_dir/daapd/music:/music
...
sudo systemctl enable daapd.service
sudo systemctl start daapd.service
