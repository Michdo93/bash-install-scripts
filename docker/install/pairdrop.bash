...
# Überprüfen, ob Pairdrop bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^pairdrop$"; then
    echo "Pairdrop ist bereits installiert."
else
    # Pairdrop Docker Compose-Datei erstellen
    compose_file="$config_dir/pairdrop.yml"
...
EOL
        ...
        echo "Pairdrop Docker Compose-Datei erstellt."
    else
        echo "Die Pairdrop Docker Compose-Datei existiert bereits."
    fi
    ...
    service_file="/etc/systemd/system/pairdrop-setup.service"
    cat > "$service_file" <<EOL
[Unit]
Description=Pairdrop Setup
...
EOL
    ...
    sudo systemctl enable pairdrop-setup.service
    sudo systemctl start pairdrop-setup.service
fi
