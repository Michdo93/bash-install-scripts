...
# Compose-Datei
compose_file="$config_dir/babybuddy.yml"

# Service-Datei
service_file="/etc/systemd/system/babybuddy.service"
...
run_command "mkdir -p $container_dir/babybuddy/appdata" "$sudo_available"
...
# Überprüfen, ob BabyBuddy bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^babybuddy$"; then
    echo "BabyBuddy ist bereits installiert."
else
...
services:
  babybuddy:
    ...
    volumes:
      - $container_dir/babybuddy/appdata:/config
    ports:
      - "$available_port:8000"
...
sudo systemctl enable babybuddy.service
