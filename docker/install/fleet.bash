...
# Compose-Datei
compose_file="$config_dir/fleet.yml"

# Service-Datei
service_file="/etc/systemd/system/fleet.service"
...
run_command "mkdir -p $conatiner_dir/fleet/config" "$sudo_available"
...
if docker ps -a --format '{{.Names}}' | grep -q "^fleet-sync$"; then
    echo "fleet ist bereits installiert."
else
...
services:
  fleet:
    ...
    volumes:
      - $conatiner_dir/fleet/config:/config
    ports:
      - "$available_port:8080"
