...
# Compose-Datei
compose_file="$config_dir/domoticz.yml"
...
run_command "mkdir -p $container_dir/domoticz/config" "$sudo_available"
run_command "mkdir -p $container_dir/domoticz/database" "$sudo_available"
...
# Überprüfen, ob domoticz bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^domoticz$"; then
    echo "Domoticz ist bereits installiert."
else
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        port1=$(find_next_port 8080)
        port2=$(find_next_port 6144)
        port3=$(find_next_port 1443)
...
services:
  domoticz:
    ...
    environment:
      ...
      - "DBASE=$container_dir/domoticz/database" #optional
    volumes:
      - $container_dir/domoticz/config:/config
    ports:
      - "$port1:8080"
      - "$port2:6144"
      - "$port3:1443"
