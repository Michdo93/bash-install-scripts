...
# Compose-Datei
compose_file="$config_dir/quassel-core.yml"

# Service-Datei
service_file="/etc/systemd/system/quassel-core.service"
...
# Überprüfen, ob Quassel Core bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^quassel-core$"; then
    echo "Quassel Core ist bereits installiert."
else
    # Quassel Core Docker Compose-Datei erstellen
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 4242)
...
EOL
        ...
        echo "Die Quassel Core Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei an Ihre Bedürfnisse an: $compose_file"
    else
        echo "Die Quassel Core Docker Compose-Datei existiert bereits: $compose_file"
    fi
fi
...
sudo systemctl enable quassel-core.service
sudo systemctl start quassel-core.service