...
# Compose-Datei
compose_file="$config_dir/unifi-controller.yml"

# Service-Datei
service_file="/etc/systemd/system/unifi-controller.service"
...
# Überprüfen, ob UniFi Controller bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^unifi-controller$"; then
    echo "UniFi Controller ist bereits installiert."
else
    # UniFi Controller Docker Compose-Datei erstellen
...
EOL
        ...
        echo "Die UniFi Controller Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei nach Bedarf an."
    else
        echo "Die Docker Compose-Datei für UniFi Controller existiert bereits: $compose_file"
    fi
fi
...
sudo systemctl enable unifi-controller.service
sudo systemctl start unifi-controller.service
