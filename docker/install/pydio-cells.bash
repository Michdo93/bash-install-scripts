...
# Compose-Datei
compose_file="$config_dir/pydio-cells.yml"

# Service-Datei
service_file="/etc/systemd/system/pydio-cells.service"
...
# Überprüfen, ob Pydio Cells bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^pydio-cells$"; then
    echo "Pydio Cells ist bereits installiert."
else
    # Pydio Cells Docker Compose-Datei erstellen
...
EOL
        ...
        echo "Pydio Cells Docker Compose-Datei erstellt: $compose_file"

        # Pydio Cells-Dienst starten
        run_command "docker-compose -f $compose_file up -d" "$sudo_available"
    else
        echo "Die Pydio Cells Docker Compose-Datei existiert bereits: $compose_file"
    fi
fi
