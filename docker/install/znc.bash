...
# Überprüfen, ob znc bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^znc$"; then
    echo "znc ist bereits installiert."
else
    # Installieren und Starten von znc
    sudo_available=$(check_sudo)
    run_command "docker run -d --name=znc --restart=unless-stopped -e PUID=1000 -e PGID=1000 -e TZ=Etc/UTC -p 6501:6501 -v $container_dir/znc/config:/config lscr.io/linuxserver/znc:latest" "$sudo_available"
fi
...
services:
  znc:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/znc/config:/config
    ports:
      - "$available_port:6501"
...
sudo systemctl enable znc-setup.service
sudo systemctl start znc-setup.service
