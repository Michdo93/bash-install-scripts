...
# Compose-Datei
compose_file="$config_dir/qdirstat.yml"

# Service-Datei
service_file="/etc/systemd/system/qdirstat.service"
...
# Überprüfen, ob qdirstat bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^qdirstat$"; then
    echo "qdirstat ist bereits installiert."
else
    # qdirstat Docker Compose-Datei erstellen
...
EOL
        ...
        echo "Die qdirstat Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei an Ihre Bedürfnisse an: $compose_file"
    else
        echo "Die qdirstat Docker Compose-Datei existiert bereits: $compose_file"
    fi
fi
...
sudo systemctl enable qdirstat.service
sudo systemctl start qdirstat.service