...
# Compose-Datei
compose_file="$config_dir/mysql-workbench.yml"

# Service-Datei
service_file="/etc/systemd/system/mysql-workbench-setup.service"
...
services:
  mysql-workbench:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/mysql-workbench/config:/config
    ports:
      - 3000:3000
      - 3001:3001
    cap_add:
...
EOL
    ...
    echo "Docker Compose-Datei für MySQL Workbench erstellt."
else
    echo "Die Docker Compose-Datei für MySQL Workbench existiert bereits."
fi
...
[Unit]
Description=MySQL Workbench Setup
...
sudo systemctl enable mysql-workbench-setup.service
sudo systemctl start mysql-workbench-setup.service

echo "MySQL Workbench wurde installiert und eingerichtet."
