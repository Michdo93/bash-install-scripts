...
# Compose-Datei
compose_file="$config_dir/wikijs-setup.yml"

# Service-Datei
service_file="/etc/systemd/system/wikijs-setup.service"
...
# Überprüfen, ob WikiJS bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^wikijs$"; then
    echo "WikiJS ist bereits installiert."
else
    # Installieren und Starten von WikiJS
    sudo_available=$(check_sudo)
    run_command "docker run -d --name=wikijs --restart=always -v $config_dir/wikijs:/config -v $config_dir/wikijs/data:/data lscr.io/linuxserver/wikijs:latest" "$sudo_available"
fi
...
services:
  wikijs:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/wikijs/config:/config
      - $container_dir/wikijs/data:/data
    ports:
      - "$available_port:3000"
...
sudo systemctl enable wikijs-setup.service
sudo systemctl start wikijs-setup.service
