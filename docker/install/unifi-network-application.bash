...
# Compose-Datei
compose_file="$config_dir/unifi-network-application.yml"

# Service-Datei
service_file="/etc/systemd/system/unifi-network-application.service"
...
# Überprüfen, ob UniFi Network Application bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^unifi-network-application$"; then
    echo "UniFi Network Application ist bereits installiert."
else
    # UniFi Network Application Docker Compose-Datei erstellen
...
sudo systemctl enable unifi-network-application.service
sudo systemctl start unifi-network-application.service