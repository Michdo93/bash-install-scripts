...
# Compose-Datei
compose_file="$config_dir/apprise-api.yml"

# Service-Datei
service_file="/etc/systemd/system/apprise-api.service"
...
run_command "mkdir -p $container_dir/apprise-api/config" "$sudo_available"
...
# Überprüfen, ob Apprise-API bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^apprise-api$"; then
    echo "Apprise-API ist bereits installiert."
else
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 8000)
...
services:
  apprise-api:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/apprise-api/config:/config
    ports:
      - "$available_port:8000"
...
sudo systemctl enable apprise-api.service
sudo systemctl start apprise-api.service
