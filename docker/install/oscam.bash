...
# Compose-Datei
compose_file="$config_dir/oscam.yml"

# Service-Datei
service_file="/etc/systemd/system/oscam-setup.service"
...
# Überprüfen, ob Oscam bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^oscam$"; then
    echo "Oscam ist bereits installiert."
else
    # Oscam Docker Compose-Datei erstellen
...
services:
  oscam:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $config_dir/oscam/config:/config
    ports:
      - "$available_port:8888"
    devices:
...
EOL
        ...
        echo "Oscam Docker Compose-Datei erstellt."
    else
        echo "Die Oscam Docker Compose-Datei existiert bereits."
    fi
...
EOL
    ...
    sudo systemctl enable oscam-setup.service
    sudo systemctl start oscam-setup.service
fi
