...
# Compose-Datei
compose_file="$config_dir/ffmpeg.yml"

# Service-Datei
service_file="/etc/systemd/system/ffmpeg-setup.service"
...
run_command "mkdir -p $container_dir/ffmpeg/config" "$sudo_available"
...
if docker ps -a --format '{{.Names}}' | grep -q "^ffmpeg-sync$"; then
...
services:
  ffmpeg:
    ...
    volumes:
      - $container_dir/ffmpeg/config:/config
    command: >
      -i /config/input.mkv
      -c:v libx264
      -b:v 4M
...
sudo systemctl enable ffmpeg-setup.service
sudo systemctl start ffmpeg-setup.service
