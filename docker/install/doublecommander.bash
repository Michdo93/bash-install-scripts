...
# Compose-Datei
compose_file="$config_dir/doublecommander.yml"

# Service-Datei
service_file="/etc/systemd/system/doublecommander.service"
...
run_command "mkdir -p $container_dir/doublecommander/config" "$sudo_available"
run_command "mkdir -p $container_dir/doublecommander/data" "$sudo_available"
...
# Überprüfen, ob Double Commander bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^doublecommander$"; then
    echo "Double Commander ist bereits installiert."
else
...
services:
  doublecommander:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/doublecommander/config:/config
      - $container_dir/doublecommander/data:/data
    ports:
      - "$port1:3000"
      - "$port2:3001"
...
sudo systemctl enable doublecommander.service
sudo systemctl start doublecommander.service
