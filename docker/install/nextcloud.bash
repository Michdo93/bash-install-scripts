...
compose_file="$config_dir/nextcloud.yml"
...
services:
  nextcloud:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/nextcloud/config:/config
      - $container_dir/nextcloud/data:/data
    ports:
      - 443:443
    restart: unless-stopped
EOL

    echo "Docker Compose-Datei für Nextcloud erstellt."
else
    echo "Die Docker Compose-Datei für Nextcloud existiert bereits."
fi
...
service_file="/etc/systemd/system/nextcloud-setup.service"
...
sudo systemctl enable nextcloud-setup.service
sudo systemctl start nextcloud-setup.service

echo "Nextcloud wurde installiert und eingerichtet."
