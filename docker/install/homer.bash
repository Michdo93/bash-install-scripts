...
# Überprüfen, ob Homer bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^homer$"; then
    echo "Homer ist bereits installiert."
else
    # Installieren und Starten von Homer
    sudo_available=$(check_sudo)
    run_command "docker run -d --name=homer --restart=unless-stopped -v /your/local/assets/:/www/assets -p 8080:8080 b4bz/homer" "$sudo_available"
fi
...
compose_file="$config_dir/homer.yml"
...
services:
  homer:
    ...
    volumes:
      - $container_dir/homer/assets:/www/assets
    ports:
      - "$available_port:8080"
...
service_file="/etc/systemd/system/homer-setup.service"
cat > "$service_file" <<EOL
[Unit]
Description=Homer Setup
...
sudo systemctl enable homer-setup.service
sudo systemctl start homer-setup.service

echo "Homer wurde erfolgreich installiert und konfiguriert."
