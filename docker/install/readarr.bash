...
# Service-Datei
service_file="/etc/systemd/system/readarr.service"
...
# Überprüfen, ob Readarr bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^readarr$"; then
...
EOL
        ...
        echo "Die Readarr Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei an Ihre Bedürfnisse an: $compose_file"
    else
        echo "Die Readarr Docker Compose-Datei existiert bereits: $compose_file"
    fi
fi
...
sudo systemctl enable readarr.service
sudo systemctl start readarr.service
