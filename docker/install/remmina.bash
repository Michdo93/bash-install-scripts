...
# Compose-Datei
compose_file="$config_dir/remmina.yml"

# Service-Datei
service_file="/etc/systemd/system/remmina.service"
...
# Überprüfen, ob Remmina bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^remmina$"; then
    echo "Remmina ist bereits installiert."
else
    # Remmina Docker Compose-Datei erstellen
...
services:
  remmina:
    ...
    ports:
      - $available_port:3000
      - $((available_port + 1)):3001
    restart: unless-stopped
EOL

        echo "Die Remmina Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei an Ihre Bedürfnisse an: $compose_file"
    else
        echo "Die Remmina Docker Compose-Datei existiert bereits: $compose_file"
    fi
...
sudo systemctl enable remmina.service
sudo systemctl start remmina.service