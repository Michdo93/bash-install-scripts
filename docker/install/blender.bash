...
# Compose-Datei
compose_file="$config_dir/blender.yml"

# Service-Datei
service_file="/etc/systemd/system/blender.service"
...
run_command "mkdir -p $container_dir/blender/config" "$sudo_available"
...
# Überprüfen, ob Blender bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^blender$"; then
    echo "Blender ist bereits installiert."
else
...
services:
  blender:
    ...
    environment:
      ...
      - SUBFOLDER=/ #optional
    volumes:
      - $container_dir/blender/config:/config
    ports:
      - "$port1:3000"
      - "$port2:3001"
