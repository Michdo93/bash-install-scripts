...
# Compose-Datei
compose_file="$config_dir/budge.yml"

# Service-Datei
service_file="/etc/systemd/system/budge.service"
...
run_command "mkdir -p $container_dir/budge/config" "$sudo_available"
...
# Überprüfen, ob Budge bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^budge$"; then
    echo "Budge ist bereits installiert."
else
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        port1=$(find_next_port 80)
        port2=$(find_next_port 443)
...
services:
  budge:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/budge/config:/config
    ports:
      - "$port1:80"
      - "$port2:443"
...
sudo systemctl enable budge.service
sudo systemctl start budge.service
