...
# Compose-Datei
compose_file="$config_dir/pylon.yml"

# Service-Datei
service_file="/etc/systemd/system/pylon.service"
...
# Überprüfen, ob Pylon bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^pylon$"; then
    echo "Pylon ist bereits installiert."
else
    # Pylon Docker Compose-Datei erstellen
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 3131)
...
EOL
        ...
        echo "Pylon Docker Compose-Datei erstellt."
    fi
fi
...
sudo systemctl enable pylon.service
sudo systemctl start pylon.service