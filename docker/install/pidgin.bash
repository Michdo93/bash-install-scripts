...
# Überprüfen, ob Pidgin bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^pidgin$"; then
    echo "Pidgin ist bereits installiert."
else
    # Pidgin Docker Compose-Datei erstellen
    compose_file="$config_dir/pidgin.yml"
...
EOL
        ...
        echo "Pidgin Docker Compose-Datei erstellt."
    else
        echo "Die Pidgin Docker Compose-Datei existiert bereits."
    fi
    ...
    service_file="/etc/systemd/system/pidgin-setup.service"
    cat > "$service_file" <<EOL
[Unit]
Description=Pidgin Setup
...
EOL
    ...
    sudo systemctl enable pidgin-setup.service
    sudo systemctl start pidgin-setup.service
fi
