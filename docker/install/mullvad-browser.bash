...
compose_file="$config_dir/mullvad-browser.yml"
...
services:
  mullvad-browser:
    ...
    volumes:
      - $container_dir/mullvad-browser/config:/config
    ports:
      - 3000:3000
      - 3001:3001
    shm_size: "1gb"
...
EOL
    ...
    echo "Docker Compose-Datei für Mullvad Browser erstellt."
else
    echo "Die Docker Compose-Datei für Mullvad Browser existiert bereits."
fi
...
service_file="/etc/systemd/system/mullvad-browser-setup.service"
cat > "$service_file" <<EOL
[Unit]
Description=Mullvad Browser Setup
...
sudo systemctl enable mullvad-browser-setup.service
sudo systemctl start mullvad-browser-setup.service

echo "Mullvad Browser wurde installiert und eingerichtet."
