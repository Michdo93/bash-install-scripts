...
# Compose-Datei
compose_file="$config_dir/changedetection.yml"
...
run_command "mkdir -p $container_dir/changedetection/config" "$sudo_available"
...
# Überprüfen, ob changedetection.io bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^changedetection$"; then
    echo "changedetection.io ist bereits installiert."
else
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 5000)
...
services:
  changedetection:
    ...
    environment:
      ...
      - BASE_URL= #optional
    volumes:
      - $container_dir/changedetection/config:/config
    ports:
      - "$available_port:5000"
