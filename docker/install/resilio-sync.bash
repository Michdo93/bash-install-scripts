...
# Compose-Datei
compose_file="$config_dir/resilio-sync.yml"

# Service-Datei
service_file="/etc/systemd/system/resilio-sync.service"
...
# Überprüfen, ob Resilio Sync bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^resilio-sync$"; then
    echo "Resilio Sync ist bereits installiert."
else
    # Resilio Sync Docker Compose-Datei erstellen
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 8888)
...
services:
  resilio-sync:
    ...
    ports:
      - $available_port:8888
      - $((available_port + 1)):55555
    restart: unless-stopped
EOL

        echo "Die Resilio Sync Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei nach Bedarf an."
    else
        echo "Die Docker Compose-Datei für Resilio Sync existiert bereits: $compose_file"
    fi
fi
