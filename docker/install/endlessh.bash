...
# Compose-Datei
compose_file="$config_dir/endlessh.yml"

# Service-Datei
service_file="/etc/systemd/system/endlessh.service"
...
run_command "mkdir -p $container_dir/endlessh/config" "$sudo_available"
...
# Überprüfen, ob endlessh bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^endlessh$"; then
    echo "endlessh ist bereits installiert."
else
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 22)
...
services:
  endlessh:
    ...
    environment:
      ...
      - BINDFAMILY= #optional
    volumes:
      - $container_dir/endlessh/config:/config #optional
    ports:
      - "$available_port:2222"
...
echo "endlessh erfolgreich installiert und eingerichtet."
