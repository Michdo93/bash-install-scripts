...
# Compose-Datei
compose_file="$config_dir/digikam.yml"

# Service-Datei
service_file="/etc/systemd/system/digikam.service"
...
run_command "mkdir -p $container_dir/digikam/config" "$sudo_available"
...
# Überprüfen, ob digikam bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^digikam$"; then
    echo "digikam ist bereits installiert."
else
...
services:
  digikam:
    ...
    environment:
      ...
      - SUBFOLDER=/ #optional
    volumes:
      - $container_dir/digikam/config:/config
    ports:
      - "$port1:3000" #optional
      - "$port2:3001" #optional
...
sudo systemctl enable digikam.service
sudo systemctl start digikam.service
