...
# Compose-Datei
compose_file="$config_dir/cops.yml"

# Service-Datei
service_file="/etc/systemd/system/cops.service"
...
run_command "mkdir -p $container_dir/cops/config" "$sudo_available"
run_command "mkdir -p $container_dir/cops/books" "$sudo_available"
...
# Überprüfen, ob cops bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^cops$"; then
    echo "cops ist bereits installiert."
else
...
services:
  cops:
    ...
    volumes:
      ...
      - $container_dir/cops/books:/books
    ports:
      - "$available_port:80"
