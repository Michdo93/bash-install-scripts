...
# Compose-Datei
compose_file="$config_dir/rdesktop.yml"

# Service-Datei
service_file="/etc/systemd/system/rdesktop.service"
...
# Überprüfen, ob rdesktop bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^rdesktop$"; then
    echo "rdesktop ist bereits installiert."
else
    # rdesktop Docker Compose-Datei erstellen
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 3389)
...
EOL
        ...
        echo "Die rdesktop Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei an Ihre Bedürfnisse an: $compose_file"
    else
        echo "Die rdesktop Docker Compose-Datei existiert bereits: $compose_file"
    fi
fi
...
sudo systemctl enable rdesktop.service
sudo systemctl start rdesktop.service
