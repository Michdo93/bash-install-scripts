...
# Überprüfen, ob Orcaslicer bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^orcaslicer$"; then
    echo "Orcaslicer ist bereits installiert."
else
    # Orcaslicer Docker Compose-Datei erstellen
    compose_file="$config_dir/orcaslicer.yml"
...
services:
  orcaslicer:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $config_dir/orcaslicer/config:/config
    ports:
      - "$available_port:3000"
      - 3001:3001
...
EOL
        ...
        echo "Orcaslicer Docker Compose-Datei erstellt."
    else
        echo "Die Orcaslicer Docker Compose-Datei existiert bereits."
    fi
    ...
    service_file="/etc/systemd/system/orcaslicer-setup.service"
    cat > "$service_file" <<EOL
[Unit]
Description=Orcaslicer Setup
...
EOL
    ...
    sudo systemctl enable orcaslicer-setup.service
    sudo systemctl start orcaslicer-setup.service
fi
