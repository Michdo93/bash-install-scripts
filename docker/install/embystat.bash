...
# Compose-Datei
compose_file="$config_dir/embystat.yml"

# Service-Datei
service_file="/etc/systemd/system/embystat.service"
...
run_command "mkdir -p $container_dir/embystat/config" "$sudo_available"
...
if docker ps -a --format '{{.Names}}' | grep -q "^emby$"; then
    ...
    if [ ! -f "$compose_file" ]; then
        ...
        available_port=$(find_next_port 6555)
...
services:
  embystat:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/embystat/config:/config
    ports:
      - "$available_port:6555"
