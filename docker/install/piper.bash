...
# Überprüfen, ob Piper bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^piper$"; then
    echo "Piper ist bereits installiert."
else
    # Piper Docker Compose-Datei erstellen
    compose_file="$config_dir/piper.yml"
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 10200)
...
services:
  piper:
    ...
    ports:
      - "$available_port:10200"
    restart: unless-stopped
EOL

        echo "Piper Docker Compose-Datei erstellt."
    else
        echo "Die Piper Docker Compose-Datei existiert bereits."
    fi
    ...
    service_file="/etc/systemd/system/piper-setup.service"
    cat > "$service_file" <<EOL
[Unit]
Description=Piper Setup
...
EOL
    ...
    sudo systemctl enable piper-setup.service
    sudo systemctl start piper-setup.service
fi
