...
# Compose-Datei
compose_file="$config_dir/sqlitebrowser.yml"

# Service-Datei
service_file="/etc/systemd/system/sqlitebrowser.service"
...
# Überprüfen, ob sqlitebrowser bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^sqlitebrowser$"; then
    echo "sqlitebrowser ist bereits installiert."
else
    # sqlitebrowser Docker Compose-Datei erstellen
...
EOL
        ...
        echo "Die sqlitebrowser Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei nach Bedarf an."
    else
        echo "Die Docker Compose-Datei für sqlitebrowser existiert bereits: $compose_file"
    fi
fi
...
sudo systemctl enable sqlitebrowser.service
sudo systemctl start sqlitebrowser.service