...
# Compose-Datei
compose_file="$config_dir/boinc.yml"

# Service-Datei
service_file="/etc/systemd/system/boinc.service"
...
run_command "mkdir -p $container_dir/boinc/config" "$sudo_available"
...
# Überprüfen, ob BOINC bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^boinc$"; then
    echo "BOINC ist bereits installiert."
else
...
services:
  boinc:
    ...
    environment:
      ...
      - PASSWORD= #optional
    volumes:
      - $container_dir/boinc/config:/config
    ports:
      - "$port1:8080"
      - "$port2:8181"
    devices:
