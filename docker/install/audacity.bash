...
# Compose-Datei
compose_file="$config_dir/audacity.yml"

# Service-Datei
service_file="/etc/systemd/system/audacity.service"
...
run_command "mkdir -p $container_dir/audacity/config" "$sudo_available"
...
# Überprüfen, ob Audacity bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^audacity$"; then
    echo "Audacity ist bereits installiert."
else
...
services:
  audacity:
    ...
    environment:
      ...
      - TZ=Etc/UTC
    volumes:
      - $container_dir/audacity/config:/config
    ports:
      - "$port1:3000"
      - "$port2:3001"
...
sudo systemctl start audacity.service
