...
# Compose-Datei
compose_file="$config_dir/bazarr.yml"
...
run_command "mkdir -p $container_dir/bazarr/config" "$sudo_available"
run_command "mkdir -p $container_dir/bazarr/movies" "$sudo_available"
run_command "mkdir -p $container_dir/bazarr/tv" "$sudo_available"
...
# Überprüfen, ob Bazarr bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^bazarr$"; then
    echo "Bazarr ist bereits installiert."
else
    if [ ! -f "$compose_file" ]; then
        # Einen verfügbaren Port finden
        available_port=$(find_next_port 6767)
...
services:
  bazarr:
    ...
    volumes:
      ...
      - $container_dir/bazarr/movies:/movies #optional
      - $container_dir/bazarr/tv:/tv #optional
    ports:
      - "$available_port:6767"
