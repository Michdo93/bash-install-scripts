...
# Compose-Datei
compose_file="$config_dir/smokeping.yml"

# Service-Datei
service_file="/etc/systemd/system/smokeping.service"
...
if docker ps -a --format '{{.Names}}' | grep -q "^smokeping$"; then
    echo "SmokePing ist bereits installiert."
else
    # SmokePing Docker Compose-Datei erstellen
...
EOL
        ...
        echo "Die SmokePing Docker Compose-Datei wurde erstellt. Bitte passen Sie die Datei nach Bedarf an."
    else
        echo "Die Docker Compose-Datei für SmokePing existiert bereits: $compose_file"
    fi
fi
