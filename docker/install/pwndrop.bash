...
# Überprüfen, ob Pwndrop bereits installiert ist
if docker ps -a --format '{{.Names}}' | grep -q "^pwndrop$"; then
    echo "Pwndrop ist bereits installiert."
else
    # Pwndrop Docker Compose-Datei erstellen
...
EOL
        ...
        echo "Pwndrop Docker Compose-Datei erstellt: $compose_file"

        # Pwndrop-Dienst starten
        run_command "docker-compose -f $compose_file up -d" "$sudo_available"
    else
        echo "Die Pwndrop Docker Compose-Datei existiert bereits: $compose_file"
    fi
fi
